...
/**
 * SyncService handles orchestration of data sync operations.
 *
 * NOTE: All sync operations are now triggered via Trigger.dev tasks
 * (see src/trigger/sync-data.ts) for durable execution, retries, and
 * observability. The methods here are kept for:
 *   - getSyncHistory() — querying the persisted sync log
 *   - logSync() — writing to the sync_log table
 *   - Direct use by Trigger.dev tasks via initServices()
 */
@Injectable()
